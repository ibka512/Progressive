...
const BottomSheet = {
    ...
    open(selectEl, textSpan) {
        ...
        let titleMap = {
            'test-range-select': 'Select Test Range',
            'test-display-select': 'Default Display Mode',
            'next-display-mode': 'Blur Mode',
            'wb-folder-filter': 'Select Folder',
            'move-dest-select': 'Move to Destination'
        };
        document.getElementById('bs-title').innerText = titleMap[selectEl.id] || "Select";
...
const Model = {
  db: [], folders: ["Default"], stars: [], records: [], editingIdx: -1,
  ...
  async loadData() {
    ...
    if (storedDB) {
        this.db = storedDB;
        this.folders = await idbKeyval.get('myFolders_v3') || ["Default"];
        ...
        if (lsDB) {
            ...
            this.folders = JSON.parse(localStorage.getItem('myFolders_v3')) || ["Default"];
          ...
          this.db = DefaultWords.map(w => ({...w, folder: "Default"})); 
          await this.saveDB(); 
        }
    }
    let needSave = false;
  ...
  checkFilter(w, filterName) {
      ...
      return w.folder === (filterName === 'default' ? 'Default' : filterName);
  },
  ...
  async speakText(text, btnEl = null) {
    try {
        ...
        if (engine === 'azure') {
            ...
            if (!response.ok) throw new Error("Azure API request failed");
        ...
        console.warn("[TTS] Online engine failed, fallback to local", e);
...
const View = {
  ...
  renderLeaderboard() {
      ...
      if (filtered.length === 0) {
          html = `<div style="text-align:center; padding: 40px 20px; opacity: 0.5;">No records yet</div>`;
      } else {
          displayList.forEach((r, idx) => {
              ...
              <div style="display:flex; justify-content:space-between; align-items:center; padding: 14px 12px; border-bottom: 1px solid var(--outline);">
                 <div>
                    <div style="font-size: 1.3rem; font-weight: 800; color: var(--primary); margin-bottom: 4px;">${r.combo} Combo</div>
  ...
  updateGroupTabs() {
      ...
      Model.folders.forEach(f => {
          let catVal = f === 'Default' ? 'default' : f;
      ...
      const virtuals = [
          { cat: 'virtual_cleared', text: 'Mastered' },
          { cat: 'virtual_uncleared', text: 'Unmastered' },
          { cat: 'virtual_miss_kanji', text: 'Miss Kanji' },
          { cat: 'virtual_miss_kana', text: 'Miss Reading' },
          { cat: 'virtual_miss_meaning', text: 'Miss Meaning' },
          { cat: 'virtual_starred', text: 'Starred' }
      ];
      virtuals.forEach(v => {
          let tab = document.createElement('div');
  ...
  renderGroupBottomSheet(cat) {
      ...
      try {
          ...
          if (words.length === 0) {
              let emptyText = "Empty here";
              let iconStr = "spa";
              let jpTitle = "【 空 無 】";
              
              if(catVal === 'virtual_starred') { emptyText = "No favorites yet. Go explore!"; }
              if(catVal === 'virtual_cleared') { emptyText = "The path is long. Keep climbing."; }
              if(catVal === 'virtual_uncleared' || catVal.includes('virtual_miss_')) { 
                  emptyText = "This dimension is fully cleared!"; 
          ...
          let catLabel = activeTabEl ? activeTabEl.innerText : (catVal === 'default' ? 'Default' : catVal);
          ...
          while (i * 10 < total) {
              ...
              btn.innerHTML = `<div style="display:flex; justify-content:space-between; align-items:center; width:100%;"><span>Words ${startIdx + 1}-${endIdx}</span>${badgeHTML}</div>`;

              let displayTxt = `${catLabel} (Words ${startIdx + 1}-${endIdx})`;
          ...
          console.error(err);
          container.innerHTML = `<div style="text-align:center; padding: 20px; color: var(--accent-red); font-size: 0.9rem;">Load error, please retry or reset</div>`;
      }
  },
  ...
  updateWordbankUI() {
    ...
    selFilter.innerHTML = ''; 
    selFilter.add(new Option('All Words', 'all'));
    selFilter.add(new Option('Starred', 'virtual_starred'));
    selFilter.add(new Option('Fully Mastered', 'virtual_cleared'));
    selFilter.add(new Option('Unmastered', 'virtual_uncleared'));
    selFilter.add(new Option('Field Guide: Kanji (Yellow)', 'virtual_know_kanji'));
    selFilter.add(new Option('Field Guide: Reading (Red)', 'virtual_know_kana'));
    selFilter.add(new Option('Field Guide: Meaning (White)', 'virtual_know_meaning'));
  ...
  renderDashboard() {
    ...
    let lastTxt = localStorage.getItem('lastCustomGroupTxt') || 'Default (Words 1-10)';
    ...
    if (btn) {
        ...
        if(isPunched) {
            ...
            btn.innerHTML = `<span class="lp-text"><span class="material-symbols-rounded" style="font-size:1.6rem;">task_alt</span> Completed</span>`;
        } else {
            btn.className = 'btn-long-press';
            btn.innerHTML = `<div class="lp-bg"></div><span class="lp-text"><span class="material-symbols-rounded" style="font-size:1.6rem;">fingerprint</span> Hold to Complete</span>`;
        }
    }
  },
  ...
  updateTestSelects() {
      ...
      let currentTest = testSel.value || localStorage.getItem('lastTestRange') || 'Default';

      testSel.innerHTML = '';
      
            let options = [
          { text: 'Default Deck', val: 'Default' },
          { text: 'Starred', val: 'virtual_starred' },
          { text: 'Mastered', val: 'virtual_cleared' },
          { text: 'Unmastered', val: 'virtual_uncleared' },
          { text: 'Missed Kanji', val: 'virtual_miss_kanji' },
          { text: 'Missed Kana', val: 'virtual_miss_kana' },
          { text: 'Missed Gloss', val: 'virtual_miss_meaning' },
          { text: 'Known Kanji', val: 'virtual_know_kanji' },
          { text: 'Known Kana', val: 'virtual_know_kana' },
          { text: 'Known Gloss', val: 'virtual_know_meaning' }
      ];

      
      Model.folders.forEach(f => {
          if(f !== 'Default') options.push({ text: f, val: f });
      });
  ...
  renderExampleBox(exString, boxId, mode = 'normal', targetWordObj = null) {
    ...
    let htmlStr = processedStr.split('||').map(blk => {
        let parts = blk.split('/'); 
        let jpPart = parts[0] ? parts[0].trim() : "No example"; 
        ...
        if (mode === 'choice' && cnPart) { 
            return `<div class="ex-item"><div class="dt-ex-jp" data-speak="${escapeHTML(pureJpText)}"><span class="material-symbols-rounded ex-speaker">volume_up</span>${safeJpPart}</div><div class="dt-ex-cn hidden-translation" data-text="${safeCnPart}"><span class="material-symbols-rounded" style="font-size:1.1rem;">lock</span> Reveal after correct answer</div></div>`; 
        }
    ...
    if (!useRuby && window.MathJax && window.MathJax.typesetPromise) { 
        window.mathJaxQueue = (window.mathJaxQueue || Promise.resolve())
            ...
            .catch((err) => { console.warn('MathJax typeset interrupted', err); });
    }
  },
  ...
  renderVirtualGrid() {
    ...
    if (filteredData.length === 0) {
        grid.innerHTML = `<div style="grid-column: 1 / -1; text-align: center; padding: 100px 20px;">
            ...
            <div style="font-size: 0.95rem; margin-top: 12px; opacity: 0.5; color: var(--on-surface);">No words found. Try another search.</div>
        </div>`;
...
const Controller = {
    ...
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.addEventListener('message', (event) => {
        if (event.data && event.data.type === 'SW_UPDATED') {
          showConfirm('Update Available', 'A new version is ready. Refresh now?', () => {
  ...
  saveSessionRecord() {
      ...
      if (mode === 'dual-track' || mode === 'memory-test' || mode === 'rote-learning') {
          ...
          Model.records.push({
              ...
              group: Model.state.currentGroupLabel || 'Default',
              timestamp: t.getTime(),
              dateStr: dateStr
          });
          Model.saveRecords();
  ...
  bindEvents() {
    ...
    let autoSpeakCheck = View.getEl('setting-auto-speak');
    if (autoSpeakCheck) { autoSpeakCheck.addEventListener('change', (e) => { Hardware.playSound('click'); Hardware.vibrate(15); localStorage.setItem('autoSpeak', e.target.checked); showToast(e.target.checked ? "Auto-speak enabled" : "Auto-speak disabled"); }); }

    let volCheck = View.getEl('setting-vol-nav');
    if (volCheck) { volCheck.addEventListener('change', (e) => { Hardware.playSound('click'); Hardware.vibrate(15); localStorage.setItem('volNav', e.target.checked); showToast(e.target.checked ? "Volume key navigation enabled" : "Volume key navigation disabled"); }); }
    ...
    if (darkBtnCheck) {
        darkBtnCheck.addEventListener('change', (e) => {
            ...
            showToast(e.target.checked ? "Translucent style enabled" : "Solid style restored");
        });
    }
    ...
    if (postponeCheck) {
        postponeCheck.addEventListener('change', (e) => {
            ...
            showToast(e.target.checked ? "Postpone tested words enabled" : "Postpone tested words disabled");
        });
    }
    ...
    if (skipCheck) {
        skipCheck.addEventListener('change', (e) => {
            ...
            showToast(e.target.checked ? "Skip mastered dimensions enabled" : "Skip mastered dimensions disabled");
        });
    }
    ...
    if (rubyCheck) {
        rubyCheck.addEventListener('change', (e) => {
            ...
            showToast(e.target.checked ? "Native Ruby rendering enabled" : "MathJax engine enabled");
    ...
    if (ttsSelectTrigger) {
        ttsSelectTrigger.addEventListener('change', (e) => {
            ...
            let names = { 'local': 'System', 'youdao': 'Youdao', 'azure': 'Azure Nanami' };
            showToast(`Switched to ${names[e.target.value]} TTS`);
        });
    }
    ...
    if (testVibrateBtn) {
        testVibrateBtn.addEventListener('click', () => {
            ...
            if (navigator.vibrate) {
                showToast('Vibration test triggered');
            } else {
    ...
    if(lpBtn) {
        lpBtn.addEventListener('pointerdown', (e) => {
            ...
            lpBtn.classList.add('pressing'); Hardware.playChargeSound(); vibrateInterval = setInterval(() => Hardware.vibrate(10), 100);
                        punchTimer = setTimeout(() => { clearPunch(); Hardware.playDingDong(); Hardware.vibrate(200); let t = new Date().toLocaleDateString('zh-CN'); Model.records.push({date: t, type: 'daily_punch'}); Model.saveRecords(); View.renderDashboard(); showToast("Daily goal logged."); }, 1500);
    ...
    View.getEl('btn-reset').addEventListener('click', () => { Hardware.vibrate(20); showConfirm('Reset to Default', 'Warning: This will erase all imported data and restore defaults.', async () => { Model.folders = ["Default"]; Model.db = DefaultWords.map(w => ({...w, folder: "Default"})); await Model.saveDB(); await Model.saveFolders(); View.updateWordbankUI(); View.resetWordbankRenderer(); Hardware.vibrate(100); }); });
  ...
  exportBackup() {
      ...
      let fileName = `Pendulum_Backup_${new Date().toLocaleDateString('zh-CN').replace(/\//g,'-')}.json`;
      let blob = new Blob([JSON.stringify(data)], {type: "application/json"});
      if (navigator.share && navigator.canShare) { let file = new File([blob], fileName, { type: "application/json" }); if (navigator.canShare({ files: [file] })) { navigator.share({ files: [file], title: 'Pendulum Backup' }).then(() => showToast("Save panel opened")).catch((e) => this.fallbackDownload(blob, fileName)); return; } }
      this.fallbackDownload(blob, fileName);
  },

  fallbackDownload(blob, fileName) { let url = URL.createObjectURL(blob); let a = document.createElement('a'); a.style.display = 'none'; a.href = url; a.download = fileName; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); showToast("Download started..."); },
      ...
      reader.onload = (e) => {
          try {
              ...
              if (data && data.db && data.folders) {
                  ...
                  Promise.all([Model.saveDB(), Model.saveFolders(), Model.saveStars(), Model.saveRecords(), Model.saveClears()]).then(() => { Hardware.playSound('success'); Hardware.vibrate(100); showToast("Restore successful!"); setTimeout(() => location.reload(), 1000); });
              } else { Hardware.playSound('error'); Hardware.vibrate(50); showToast("Invalid backup file"); }
          } catch(err) { Hardware.playSound('error'); Hardware.vibrate(50); showToast("Failed to parse file"); }
      };
  ...
  startPendulum(launchMode = 'pendulum') {
    ...
    if (!Model.state.currentGroupLabel) { Model.state.currentGroupLabel = localStorage.getItem('lastCustomGroupTxt') || 'Default (Words 1-10)'; }
    ...
    if(sourceWords.length === 0) return showToast("No words in selected range");
    Hardware.playSound('click'); 
  ...
  startFilterTest() {
      ...
      if (sourceWords.length === 0) {
          return showConfirm('Dimension Complete', 'All bars for this mode are lit. Go to Settings to disable "Skip Mastered"?', () => {
              Nav.switchTab('tab-settings', ' |Settings', document.querySelector('[data-target="tab-settings"]'));
          });
      }
      Hardware.playSound('click'); 
  ...
  mtAdvanceNext() { 
      if (Model.state.mode === 'memory-test') {
          ...
          if (Model.state.studyQueue.length === 0) {
              if (Model.state.mtRound < 3) { Model.state.mtRound++; Model.state.studyQueue = [...Model.state.mtBaseQueue].sort(() => Math.random() - 0.5); Hardware.playSound('success'); Hardware.vibrate(200); showToast(`Round ${Model.state.mtRound - 1} cleared! Advancing`); setTimeout(() => View.renderStudyCard('next'), 500); } 
              else { 
  ...
  finishPendulum() {
      ...
      showToast("Set mastered."); View.getEl('btn-exit-study').click();
  },
  ...
  createFolder() { Hardware.vibrate(20); showPrompt("New Folder Name", "", (name) => { if(Model.folders.includes(name)) return showToast("Folder already exists"); Model.folders.push(name); Model.saveFolders(); View.updateWordbankUI(); }); },
  deleteFolder() { 
      Hardware.vibrate(20); 
      let filter = View.getEl('wb-folder-filter').value; 
      if (filter === 'all' || filter === 'Default' || filter.startsWith('virtual_')) return showToast("Cannot delete built-in category"); 
      showConfirm('Delete Folder', `Delete "${filter}"? Words inside will be moved to Default.`, () => { 
          if (Model.state.batchMode) this.toggleBatchMode();
          Model.db.forEach(w => { if(w.folder === filter) w.folder = "Default"; }); 
  ...
  openMoveModal(idx) { 
      if (idx === -2 && Model.state.selectedSet.size === 0) return showToast("No words selected"); 
  ...
  batchDelete() { 
      ...
      if(Model.state.selectedSet.size === 0) return showToast("No words selected"); 
      showConfirm('Batch Delete', `Delete ${Model.state.selectedSet.size} words?`, () => { 
  ...
  deleteWord(idx) { 
      showConfirm('Delete Word', 'Permanently delete this word?', () => { 
          this.closeDetailIfOpen();
  ...
  importWords() { 
      ...
      if(target === 'all' || target.startsWith('virtual_')) target = "Default"; 
      let added = 0; 
      text.split('\n').forEach(line => { 
      ...
      if(anim !== 'none') { 
          ...
          setTimeout(() => { 
              try {
                  ...
                  console.error('Failed to update detail content', err);
